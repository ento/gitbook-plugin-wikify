...
test('.index ignores README.md', async (t) => {
...
test('.generateIndexPages copies README.md if exists in a directory', async t => {
  const root = uniqueTempDir({create: true, thunk: true});
  fs.ensureDirSync(root('a/b'));
  touch(root('index.md'));
  touch(root('a/index.md'));
  touch(root('a/b/index.md'));
  await Wiki.index(root())
          .then(wiki => {
            return Wiki.generateIndexPages(wiki);
          });
  t.truthy(fs.existsSync(root('a/README.md')));
  t.truthy(fs.existsSync(root('a/b/README.md')));
});
...
test('.generateIndexPages lists subdirectories', async t => {
  ...
  t.truthy(fs.existsSync(root('a/README.md')));
  const aIndex = fs.readFileSync(root('a/README.md'), 'utf-8');
  t.regex(aIndex, /\[foo\]\(\.\/foo\/README.md\)/);
  t.regex(aIndex, /\[bar\]\(\.\/bar\/README.md\)/);
});
...
test('.generateSummaryPage lists directories', async t => {
  ...
  t.truthy(fs.existsSync(root('SUMMARY.md')));
  const summary = fs.readFileSync(root('SUMMARY.md'), 'utf-8');
  t.regex(summary, /\[a\]\(a\/README\.md\)/);
  t.regex(summary, /\[a\/b\]\(a\/b\/README\.md\)/);
});
