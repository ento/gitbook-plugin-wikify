...
test('finish: reports an orphaned page', async t => {
  ...
  const lintOutputPath = tempDir('wikilint.xml');
  const config = {
    'lintOutput': lintOutputPath,
    'lintMessageNoInboundLink': 'noinboundlink'
  };
  const result = await autoindexedBuilder()
        .withContent('')
        .withPage('hello', 'world')
        .withBookJson({'pluginsConfig': {'wikify': config}})
        .withLogger(logger)
        .create()
        .then(() => {
          checkTapResult(logger, (result) => {
  ...
  t.regex(result, /noinboundlink/);
});

test('finish: reports an orphaned directory index even with a child page', async t => {
  // a child page has breadcrumb links to ancestors; test that those links
  // don't make a directory index not orphaned
  const logger = memoryLogger();
  const tempDir = uniqueTempDir({create: true, thunk: true});
  const lintOutputPath = tempDir('wikilint.xml');
  const result = await autoindexedBuilder()
...
test('finish: does not report an orphaned directory if no child pages', async t => {
  const tempDir = uniqueTempDir({create: true, thunk: true});
  const lintOutputPath = tempDir('wikilint.xml');
  const config = {
    'lintOutput': lintOutputPath
  ...
  const result = await autoindexedBuilder()
        .withContent('')
        .withFile('a/hello.png', 'world')
        .withBookJson({'pluginsConfig': {'wikify': config}})
        .create()
        .then(() => {
          return fs.readFileAsync(lintOutputPath, 'utf-8');
        });
  t.notRegex(result, /"a\/README\.md"/);
});

test('finish: does not report an orphaned page if excluded', async t => {
  const tempDir = uniqueTempDir({create: true, thunk: true});
  const lintOutputPath = tempDir('wikilint.xml');
  const config = {
    'lintOutput': lintOutputPath,
    'lintExcludePatterns': ['a/**/*']
  };
  const result = await autoindexedBuilder()
        .withContent('')
        .withPage('a/b/hello', '---\nvar: hello\n---\nworld')
  ...
  t.notRegex(result, /hello\.md/);
});

test('finish: does not report an orphaned directory index if excluded', async t => {
  const tempDir = uniqueTempDir({create: true, thunk: true});
  const lintOutputPath = tempDir('wikilint.xml');
  const config = {
    'lintOutput': lintOutputPath,
