...
module.exports = {
  hooks: {
    ...
    'finish': function() {
      ...
      // todo: want to pull this out but not sure where to put
      // hasValidInboundLinks and autogeneratedPaths.
      const lintOutputPath = this.config.get('pluginsConfig.wikify.lintOutput');
      if (typeof lintOutputPath === 'string') {
        const readmePath = this.config.get('structure.readme');
        const excludePatterns = this.config.get('pluginsConfig.wikify.lintExcludePatterns');
        const lintMessageNoInboundLink = this.config.get('pluginsConfig.wikify.lintMessageNoInboundLink');

        let excludeMatcher = null;
        if (excludePatterns.length == 1) {
          excludeMatcher = new Minimatch(excludePatterns[0]);
        } else if (excludePatterns.length > 1) {
          excludeMatcher = new Minimatch('{' + excludePatterns.join(',') + '}');
        }

        const lintMessages = Array.from(paths).map(path => {
          if (path === readmePath) return;
          if (excludeMatcher !== null && excludeMatcher.match(path)) return;
          ...
          if (!hasValidInboundLinks(path)) {
            return {
              ...
              messages: [
                {
                  message: lintMessageNoInboundLink,
                  source: path,
                  line: 1,
                  column: 1
                }
